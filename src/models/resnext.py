...
class Block(nn.Module):
    def __init__(self, in_planes, cardinality, bottleneck_width, stride):
        ...
        self.conv3 = nn.Conv2d(D, 4 * D, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(4 * D)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != 4 * D:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, 4 * D, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(4 * D)
            )
...
class ResNeXt(nn.Module):
    def __init__(self, num_blocks, cardinality=8, bottleneck_width=64, num_classes=100):
        super().__init__()
        ...
        self.linear = nn.Linear(cardinality * bottleneck_width * 4, num_classes)
    ...
    def _make_layer(self, num_blocks, stride):
        ...
        for s in strides:
            layers.append(Block(self.in_planes, self.cardinality,
                                self.bottleneck_width, s))
            self.in_planes = self.cardinality * self.bottleneck_width * 4
